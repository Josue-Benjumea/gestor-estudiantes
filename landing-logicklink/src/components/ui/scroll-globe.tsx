...
import TechScene from "@/components/ui/tech-scene";
  ...
  return (
    <div ref={containerRef} className={cn("relative w-full max-w-screen overflow-x-hidden min-h-screen bg-background text-foreground", className)}>
      ...
      <div className="fixed top-0 left-0 w-full h-0.5 bg-gradient-to-r from-border/20 via-border/40 to-border/20 z-50">
        <div
          className="h-full bg-gradient-to-r from-primary via-purple-500 to-purple-900 will-change-transform shadow-sm"
          style={{
      ...
      {/* Tech Scene — Laptop → Server → Cloud */}
      <div
        ...
        style={{
          ...
          filter: `opacity(${activeSection === sections.length - 1 ? 0.35 : 0.85})`,
        }}
      >
        <div className="scale-75 sm:scale-90 lg:scale-100">
          <TechScene activeSection={activeSection} />
        </div>
      </div>
      ...
      {sections.map((section, index) => (
        ...
        >
          <div className={cn("w-full max-w-sm sm:max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl will-change-transform transition-all duration-700")}>
            <h1 className={cn(
              "font-bold mb-6 sm:mb-8 leading-[1.1] tracking-tight",
              index === 0 ? "text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl" : "text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl 2xl:text-7xl"
            )}>
              {section.subtitle ? (
                <div className="space-y-1 sm:space-y-2">
                  <div className="gradient-text-hero">{section.title}</div>
            ...
            <div className={cn(
              ...
              {index === 0 && (
                <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm text-muted-foreground/60 mt-4 sm:mt-6">
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <div className="glow-dot" style={{ width: 4, height: 4, animationDelay: '0.5s' }} />
            ...
            {section.features && (
              <div className="grid gap-3 sm:gap-4 mb-8 sm:mb-10">
                {section.features.map((feature, fi) => (
                  <div key={feature.title} className="glass-card group p-4 sm:p-5 lg:p-6 hover:-translate-y-1" style={{ animationDelay: `${fi * 0.1}s` }}>
            ...
            {section.actions && (
              <div className={cn("flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4", section.align === 'center' && "justify-center")}>
                {section.actions.map((action) => (
                  <button key={action.label} onClick={action.onClick} className={action.variant === 'primary' ? 'btn-primary' : 'btn-secondary'}>
                    {action.label}
                  </button>
                ))}
              </div>
